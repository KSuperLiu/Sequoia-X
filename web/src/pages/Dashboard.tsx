...
import { confidenceLabel, Kpi, PageHeader, StatusBadge, ZoneBadge } from "../components/Ui";
...
export default function Dashboard() {
  ...
  const visibleRisks = data.risk_alerts.filter((item) => isAdmin || item.to !== "/tasks");
  return <>
    <PageHeader title="核心决策工作台" subtitle={data.run ? `${data.run.trade_date} 收盘数据 · 决策、计划与账户风险一屏掌握` : "尚未生成追踪日报"} actions={<><span className={`freshness ${data.data_stale ? "stale" : "fresh"}`}>{data.data_stale ? "数据待更新" : "数据已校验"}</span>{isAdmin && <button className="primary icon-button" onClick={trigger} disabled={data.latest_job?.status === "RUNNING" || data.latest_job?.status === "PENDING"}><RefreshCw size={15} />{data.latest_job?.status === "RUNNING" ? "更新中" : "更新数据"}</button>}</>} />
    {data.data_stale && <div className="warning-banner"><ShieldAlert size={16} /> 行情数据未通过新鲜度校验，可查看历史结果，但不可执行仓位建议。{isAdmin && <button onClick={() => navigate("/tasks")}>查看任务</button>}</div>}
