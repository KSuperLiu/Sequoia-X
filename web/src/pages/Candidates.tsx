...
function CandidateTable() {
  ...
  return <>
    ...
    <section className="table-panel">{loading ? <div className="skeleton">正在筛选候选…</div> : data.items.length === 0 ? <Empty>当前条件没有候选，请调整筛选或完成一次数据更新。</Empty> : <div className="table-scroll"><table className="candidate-table"><thead><tr>{isAdmin && <th><input type="checkbox" checked={selected.length === data.items.length && data.items.length > 0} onChange={(e) => setSelected(e.target.checked ? data.items.map((row) => row.id) : [])} /></th>}<th>标的</th><th>赛道</th><th>现价 / PE</th><th>回撤</th><th>反弹</th><th>量比</th><th>总分</th><th>共振策略</th><th>位置</th><th>买入区间</th><th>止损</th>{filters.account_id && <th>建议股数</th>}<th>生命周期</th><th>计划</th></tr></thead><tbody>{data.items.map((row) => <tr key={row.id} onClick={() => navigate(`/stocks/${row.symbol}`)}>{isAdmin && <td onClick={(e) => e.stopPropagation()}><input type="checkbox" checked={selected.includes(row.id)} onChange={(e) => setSelected((old) => e.target.checked ? [...old, row.id] : old.filter((id) => id !== row.id))} /></td>}<td><b>{row.name || row.symbol}</b><small>{row.symbol}</small></td><td>{row.industry || "—"}</td><td><b>{money(row.real_close)}</b><small>PE {money(row.pe_ttm)}</small></td><td className="market-down">-{pct(row.drawdown_60)}</td><td className="market-up">+{pct(row.rebound_60)}</td><td>{row.volume_ratio?.toFixed(2)}</td><td><span className={`score score-${row.total_score}`}>{row.total_score}</span></td><td>{row.consensus_count}<small>{row.strategies.map(strategyLabel).join(" / ")}</small><small>{row.confidence}</small></td><td><ZoneBadge zone={row.current_zone || row.zone} /></td><td>{money(row.current_entry_low || row.entry_low)} – {money(row.current_entry_high || row.entry_high)}</td><td className="negative">{money(row.current_stop_price || row.stop_price)}</td>{filters.account_id && <td><b>{row.suggested_quantity || 0}</b></td>}<td>{row.lifecycle_status}</td><td><StatusBadge status={row.plan_status} /></td></tr>)}</tbody></table></div>}<footer className="pagination"><span>共 {data.total} 条 · 第 {data.page} 页</span><div><button disabled={data.page <= 1} onClick={() => update("page", String(data.page - 1))}><ChevronLeft size={16} /></button><button disabled={data.page * data.page_size >= data.total} onClick={() => update("page", String(data.page + 1))}><ChevronRight size={16} /></button></div></footer></section>
  </>;
}
