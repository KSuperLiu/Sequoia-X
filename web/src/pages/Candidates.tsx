...
import { confidenceLabel, Empty, Modal, PageHeader, StatusBadge, ZoneBadge } from "../components/Ui";
import { StrategyHelpButton } from "../components/StrategyGuide";
import { useToast } from "../components/Toast";
import { STRATEGIES, strategyLabel } from "../strategies";
...
function marketCap(value?: number) {
  ...
  return `${yi >= 1_000 ? yi.toFixed(0) : yi.toFixed(1)}亿`;
}

function industryName(value?: string) {
  return value?.replace(/^[A-Z]\d{2}\s*[-—_/·:：]?\s*/i, "").trim() || "—";
}
...
export default function Candidates() {
  ...
  return <><PageHeader title="机会中心" subtitle="策略候选与独立自选统一跟踪，所有信号、计划和后续表现持续留痕" actions={<div className="page-actions">{tab === "candidates" && <StrategyHelpButton />}<div className="segmented"><button className={tab === "candidates" ? "active" : ""} onClick={() => setTab("candidates")}>策略候选</button><button className={tab === "watchlist" ? "active" : ""} onClick={() => setTab("watchlist")}><Star size={14} />自选股</button></div></div>} />{tab === "candidates" ? <CandidateTable /> : <Watchlist />}</>;
}
...
function CandidateTable() {
  ...
  const persistFilters = (next: typeof filters) => {
    const defaults: typeof filters = { q: "", strategy: "", zone: "", min_score: "0", lifecycle: "", plan_status: "", confidence: "", sort: "score_desc", account_id: "", page: "1" };
  ...
  const reset = () => {
    const next = { q: "", strategy: "", zone: "", min_score: "0", lifecycle: "", plan_status: "", confidence: "", sort: "score_desc", account_id: "", page: "1" };
    setFilters(next);
    setSearchParams({}, { replace: true });
  };
  return <>
    <section className="filter-panel"><div className="search-field"><Search size={15} /><input value={filters.q} onChange={(e) => update("q", e.target.value)} placeholder="代码或名称" /></div><select aria-label="策略筛选" value={filters.strategy} onChange={(e) => update("strategy", e.target.value)}><option value="">全部策略</option>{STRATEGIES.map((strategy) => <option key={strategy.key} value={strategy.key}>{strategy.name}</option>)}</select><select aria-label="位置筛选" value={filters.zone} onChange={(e) => update("zone", e.target.value)}><option value="">全部位置</option><option value="LEFT">左侧</option><option value="MIDDLE">中部</option><option value="RIGHT,VETO">右侧 / 否决</option><option value="RIGHT">仅右侧</option><option value="VETO">仅否决</option></select><select value={filters.min_score} onChange={(e) => update("min_score", e.target.value)}><option value="0">全部分数</option><option value="5">5 分以上</option><option value="6">6 分以上</option><option value="7">7 分以上</option></select><select value={filters.confidence} onChange={(e) => update("confidence", e.target.value)}><option value="">全部置信度</option><option value="HIGH">高置信</option><option value="MEDIUM">中等</option><option value="LOW">低</option></select><select value={filters.lifecycle} onChange={(e) => update("lifecycle", e.target.value)}><option value="">全部生命周期</option><option value="NEW">新信号</option><option value="WATCHING">关注中</option><option value="PLANNED">已计划</option><option value="BOUGHT">已买入</option><option value="DROPPED">已放弃</option><option value="EXPIRED">已过期</option></select><select value={filters.plan_status} onChange={(e) => update("plan_status", e.target.value)}><option value="">全部计划</option><option value="DRAFT">草稿</option><option value="READY">待执行</option><option value="EXECUTED">已执行</option><option value="CANCELLED">已取消</option><option value="EXPIRED">已过期</option></select><select value={filters.account_id} onChange={(e) => update("account_id", e.target.value)}><option value="">不计算仓位</option>{accounts.map((account) => <option key={account.id} value={account.id}>{account.name}</option>)}</select><select value={filters.sort} onChange={(e) => update("sort", e.target.value)}><option value="score_desc">分数从高到低</option><option value="score_asc">分数从低到高</option><option value="price_desc">价格从高到低</option></select><button className="secondary icon-button" onClick={reset}><FilterX size={15} />重置</button><a className="secondary icon-button" href={`/api/v1/candidates/export.csv?zone=${encodeURIComponent(filters.zone)}&min_score=${filters.min_score}`}><Download size={15} />导出</a></section>
    {isAdmin && selected.length > 0 && <div className="bulk-bar"><b>已选择 {selected.length} 条</b><button onClick={() => bulk("WATCHING")}>加入关注</button><button onClick={() => bulk("PLANNED")}>转为计划</button><button className="danger-text" onClick={() => bulk("DROPPED")}>放弃</button><button onClick={() => setSelected([])}>取消选择</button></div>}
    <section className="table-panel">{loading ? <div className="skeleton">正在筛选候选…</div> : data.items.length === 0 ? <Empty>当前条件没有候选，请调整筛选或完成一次数据更新。</Empty> : <div className="table-scroll"><table className="candidate-table"><thead><tr>{isAdmin && <th><input type="checkbox" checked={selected.length === data.items.length && data.items.length > 0} onChange={(e) => setSelected(e.target.checked ? data.items.map((row) => row.id) : [])} /></th>}<th>标的</th><th className="industry-column">赛道</th><th>总市值</th><th>现价 / PE</th><th>回撤</th><th>反弹</th><th>量比</th><th>总分</th><th>共振策略</th><th>位置</th><th>买入区间</th><th>止损</th>{filters.account_id && <th>建议股数</th>}<th>生命周期</th><th>计划</th></tr></thead><tbody>{data.items.map((row) => <tr key={row.id} onClick={() => navigate(`/stocks/${row.symbol}`)}>{isAdmin && <td onClick={(e) => e.stopPropagation()}><input type="checkbox" checked={selected.includes(row.id)} onChange={(e) => setSelected((old) => e.target.checked ? [...old, row.id] : old.filter((id) => id !== row.id))} /></td>}<td><b>{row.name || row.symbol}</b><small>{row.symbol}</small></td><td className="industry-column"><span className="industry-label" title={industryName(row.industry)}>{industryName(row.industry)}</span></td><td className="market-cap-cell"><b>{marketCap(row.market_cap)}</b></td><td><b>{money(row.real_close)}</b><small>PE {money(row.pe_ttm)}</small></td><td className="market-down">-{pct(row.drawdown_60)}</td><td className="market-up">+{pct(row.rebound_60)}</td><td>{row.volume_ratio?.toFixed(2)}</td><td><span className={`score score-${row.total_score}`}>{row.total_score}</span></td><td>{row.consensus_count}<small>{row.strategies.map(strategyLabel).join(" / ")}</small><small>{confidenceLabel(row.confidence)}</small></td><td><ZoneBadge zone={row.current_zone || row.zone} /></td><td>{money(row.current_entry_low || row.entry_low)} – {money(row.current_entry_high || row.entry_high)}</td><td className="negative">{money(row.current_stop_price || row.stop_price)}</td>{filters.account_id && <td><b>{row.suggested_quantity || 0}</b></td>}<td>{row.lifecycle_status}</td><td><StatusBadge status={row.plan_status} /></td></tr>)}</tbody></table></div>}<footer className="pagination"><span>共 {data.total} 条 · 第 {data.page} 页</span><div><button disabled={data.page <= 1} onClick={() => update("page", String(data.page - 1))}><ChevronLeft size={16} /></button><button disabled={data.page * data.page_size >= data.total} onClick={() => update("page", String(data.page + 1))}><ChevronRight size={16} /></button></div></footer></section>
  </>;
}
...
function Watchlist() {
  ...
  return <><div className="section-actions"><div><b>独立自选股</b><span>目标价和关注价在日跑后检查，并写入飞书日报摘要</span></div>{user && <button className="primary icon-button" onClick={() => setShow(true)}><Plus size={15} />添加自选</button>}</div><section className="table-panel">{rows.length === 0 ? <Empty>{user ? "还没有自选股，可搜索完整本地股票池添加。" : "暂无自选股数据。"}</Empty> : <div className="table-scroll"><table><thead><tr><th>标的</th><th>分组</th><th>最新价</th><th>目标价</th><th>关注价</th><th>状态</th><th>备注</th>{user && <th>操作</th>}</tr></thead><tbody>{rows.map((row) => <tr key={row.id} className="click-row" onClick={() => navigate(`/stocks/${row.symbol}`)}><td><b>{row.name || row.symbol}</b><small>{row.symbol} · {row.industry || "—"}</small></td><td>{row.group_name}</td><td>{money(row.close)}<small>{row.quote_date || "等待日跑"}</small></td><td>{money(row.target_price)}</td><td>{money(row.watch_price)}</td><td>{row.alert === "TARGET" ? <span className="status succeeded">已达目标</span> : row.alert === "WATCH" ? <span className="status failed">跌破关注</span> : <span className="status">跟踪中</span>}</td><td>{row.note || "—"}</td>{user && <td><div className="row-actions"><button title="编辑" onClick={(e) => { e.stopPropagation(); setEditing(row); }}><Edit3 size={15} /></button><button className="danger-text" title="移除" onClick={(e) => { e.stopPropagation(); void remove(row); }}><Trash2 size={15} /></button></div></td>}</tr>)}</tbody></table></div>}</section>{user && show && <Modal title="添加自选股" onClose={() => setShow(false)}><WatchForm onSaved={() => { setShow(false); load(); }} /></Modal>}{user && editing && <Modal title={`编辑自选 · ${editing.name || editing.symbol}`} onClose={() => setEditing(null)}><WatchEditForm item={editing} onSaved={() => { setEditing(null); load(); }} /></Modal>}</>;
}
